...
class SortPostsByComments(Resource):
  def get(self):
    ...
    # post.comments is a list of objects. len(post.comments) is the criteria for the sorting method.
...
class MostPopularCommenter(Resource):
  def get(self):
    ...
    for comment in comments:
      commenters.append(comment.commenter) #list with commenters
    
    dicts = []
    for commenter in set(commenters): # collection of unique commenters
      count = commenters.count(commenter)
      dicts.append({commenter: count})
    
    dicts.sort(reverse=True, key=lambda dict: list(dict.values())[0]) # will sort the dicts in descending order by their values. The [0] will extract each value from its list [2][3][1] => 2, 3, 1. Then, the sort method will sort the dicts based on those numbers.
    return {'Commenter': list(dicts[0].keys())[0]} # had to make a list to extract the key from the dict_keys object
